...
Description
  ...
  The HaskellExt backend is a Python XML-RPC service, which provides -  
...
Prerequisites
  ...
  * A UNIX or UNIX-like operating system, e.g., NetBSD, Solaris, Mac
    ...
    The HaskellExt backend does run on Windows (with some 
  ...
  * Haskell backend since HaskellExt is derived from it.
...
Quick Start
  ...
  HaskellExt backend using the following command (you may have to 
  specify the path)::

    backendctl -u root -p bazquux HaskellExt start
  ...
  or        
    ...
    backendctl -u root -p bazquux HaskellExt status

  and get a response similar to::

    {'queue': 0, 'backends': ['haskellext'], 'pid': 2642, 'results': 0}
  ...
  Testing a Haskell program with the HaskellExt backend requires a 
...
Credits
  ...
  The HaskellExt backend was written by "Mario 
...
License
  ...
  The HaskellExt backend is distributed in the hope that it will be
